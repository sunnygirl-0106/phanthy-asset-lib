 ...
 *   #/team                  团队资产库
 *   #/plaza                 素材广场
 *   #/project/:pid          项目工作台（模式选择 / 概览）
 *   #/project/:pid/assets   项目资产库
 *   #/project/:pid/workflow 工作流
 *   #/project/:pid/canvas   无限画布
 *   #/review                审核中心（头像菜单进入）
 */

import { useSyncExternalStore, useCallback } from 'react'

/** 项目工作台下的子页签。overview = 模式选择/概览。 */
export type ProjectTab = 'overview' | 'assets' | 'workflow' | 'canvas'
  ...
  switch (seg[0]) {
    ...
    case 'project': {
      ...
      const tab: ProjectTab =
        tabSeg === 'assets' || tabSeg === 'workflow' || tabSeg === 'canvas' ? tabSeg : 'overview'
...
export function routeToHash(route: Route): string {
  switch (route.name) {
    case 'project':
      return route.tab === 'overview' ? `#/project/${route.pid}` : `#/project/${route.pid}/${route.tab}`
    default:
      return `#/${route.name}`
  }
}
