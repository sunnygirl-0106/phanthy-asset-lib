 ...
 * 【0814 · 退出确认】资产「定义层」快照。
 *
 * 详情页把一份资产上的字段分成两层，语义完全不同，不要合并：
 *
 *   · 素材层 —— candidates（本次生成 / 上传 / 从库添加的图）。
 *     用户花了星钻和时间换来的产出物，进来即落库、**永远保留**，
 *     不受「保存 / 不保存」管辖，退出时也不拦。
 *
 *   · 定义层 —— 下面这几个字段，回答的是「这份资产对外是什么」。
 *     它们会被画布节点引用、被同项目的人看到，所以需要用户显式表态。
 *     退出前若有未保存的定义层改动，弹一次确认逐条列出；
 *     用户选「放弃修改」就还原回这份快照（见 store.revertAssetFields）。
 *
 * 只收窄到用户在详情里能直接改的那几个字段——scope / status / masterId
 * 这些不是用户在这个界面上改的，不进快照，免得放弃修改时误伤。
 */
